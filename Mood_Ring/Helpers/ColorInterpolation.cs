...
// 설명: HSV 색공간 기반 색상 보간 유틸리티. 두 색의 전환을 부드럽게 처리(휴 최단거리 회전).
...
public static class ColorInterpolation
{
    ...
    public static MediaColor HsvToRgb(double h, double s, double v)
    {
        ...
        return MediaColor.FromRgb((byte)Math.Round(r * 255), (byte)Math.Round(g * 255), (byte)Math.Round(b * 255));
    }
}
